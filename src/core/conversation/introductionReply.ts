...
/**
 * Matches a standalone self-introduction ("Hello, my name is X." / "I'm
 * X.") and returns the captured name, sanitized — or undefined if `text`
 * doesn't look like one. Shared by introductionReply below (1A's own
 * in-lesson exchange) and orchestrator.ts's pre-kickoff name-capture
 * prompt (see ConversationOrchestrator.startLesson's captureNameFirst),
 * so both recognize a name the exact same way.
 *
 ...
 */
export function extractIntroducedName(text: string): string | undefined {
...
export function introductionReply(lessonCode: string, messages: Message[]): TutorResponse | undefined {
  ...
  const name = extractIntroducedName(latest.content);
  if (!name) return;
  return {
    speech: {
